...
def compute_rhs(u):
    ...
    j = 0
    solndat.netfluxu[j] = (u[j + 1] ** 2 + u[j + 1] * u[j] - u[j] * ujm1 - ujm1 ** 2) / pdat.dx6 #- pdat.vdx2 * (u[j + 1] - 2 * u[j] + ujm1)

    j = pdat.dim - 1
    solndat.netfluxu[j] = (ujp1 ** 2 + ujp1 * u[j] - u[j] * u[j - 1] - u[j - 1] ** 2) / pdat.dx6 #- pdat.vdx2 * (ujp1 - 2 * u[j] + u[j - 1])
