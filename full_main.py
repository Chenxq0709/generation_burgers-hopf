...
def compute_rhs(u):
    ...
    for j in range(1, pdat.dim - 1):
        solndat.netfluxu[j] = (u[j + 1] ** 2 + u[j + 1] * u[j] - u[j] * u[j - 1] - u[
            j - 1] ** 2) / pdat.dx6 - pdat.vdx2 * (u[j + 1] - 2 * u[j] + u[j - 1])

    j = 0
    solndat.netfluxu[j] = (u[j + 1] ** 2 + u[j + 1] * u[j] - u[j] * ujm1 - ujm1 ** 2) / pdat.dx6 - pdat.vdx2 * (
    u[j + 1] - 2 * u[j] + ujm1)

    j = pdat.dim - 1
    solndat.netfluxu[j] = (ujp1 ** 2 + ujp1 * u[j] - u[j] * u[j - 1] - u[j - 1] ** 2) / pdat.dx6 - pdat.vdx2 * (
    ujp1 - 2 * u[j] + u[j - 1])

    solndat.netfluxu = -solndat.netfluxu + solndat.force
...
def compute_rhs_slow(xu):
    ...
    for j in range(1, pdat.dimslow - 1):
        solndat.netfluxxu[j] = (xu[j + 1] ** 2 + xu[j + 1] * xu[j] - xu[j] * xu[j - 1] - xu[
            j - 1] ** 2) / pdat.ndx6 - pdat.vndx2 * (xu[j + 1] - 2 * xu[j] + xu[j - 1])

    j = 0
    solndat.netfluxxu[j] = (
                           xu[j + 1] ** 2 + xu[j + 1] * xu[j] - xu[j] * xujm1 - xujm1 ** 2) / pdat.ndx6 - pdat.vndx2 * (
    xu[j + 1] - 2 * xu[j] + xujm1)

    j = pdat.dimslow - 1
    solndat.netfluxxu[j] = (
                           xujp1 ** 2 + xujp1 * xu[j] - xu[j] * xu[j - 1] - xu[j - 1] ** 2) / pdat.ndx6 - pdat.vndx2 * (
    xujp1 - 2 * xu[j] + xu[j - 1])
...
def compute_rhs_average(U):
    ...
    for j in range(1, pdat.dimslow - 1):
        solndat.netfluxU[j] = (U[j + 1] ** 2 + U[j + 1] * U[j] - U[j] * U[j - 1] - U[
            j - 1] ** 2) / pdat.ndx6 - pdat.vndx22 * (U[j + 1] - 2 * U[j] + U[j - 1])

    j = 0
    solndat.netfluxU[j] = (U[j + 1] ** 2 + U[j + 1] * U[j] - U[j] * Ujm1 - Ujm1 ** 2) / pdat.ndx6 - pdat.vndx22 * (
    U[j + 1] - 2 * U[j] + Ujm1)

    j = pdat.dimslow - 1
    solndat.netfluxU[j] = (Ujp1 ** 2 + Ujp1 * U[j] - U[j] * U[j - 1] - U[j - 1] ** 2) / pdat.ndx6 - pdat.vndx22 * (
    Ujp1 - 2 * U[j] + U[j - 1])
...
def force_term():
    ...
    for j in range(0, pdat.dimslow):
        solndat.xforce[j] = pdat.Adt * (solndat.alpha[0] * np.cos(pdat.tpi * (j * pdat.dxl + solndat.phi[0])) + \
                                        solndat.alpha[1] / np.sqrt(2) * np.cos(
                                            pdat.tpi * (2 * j * pdat.dxl + solndat.phi[1])) + \
                                        solndat.alpha[2] / np.sqrt(3) * np.cos(
                                            pdat.tpi * (3 * j * pdat.dxl + solndat.phi[2])))
...
for j in range(pdat.nums_skip):
    ...
    make_one_step_RK3()
    pdat.advance_time()
    pdat.PrintMesProgress(solndat.u, pdat.dt)
...
if compute_stat:
    ...
    statdat.record_cfsoln(solndat.u, pdat)
...
for j in range(pdat.nums):
    ...
    pdat.PrintMesProgress(solndat.u, pdat.dtsubs)
    ...
    if compute_stat:
        ...
        # compute CF
        statdat.compute_cft(solndat.u, pdat)
